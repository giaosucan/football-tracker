...
public class TimeTick implements Chronometer.OnChronometerTickListener {

    TextView clockFace; // The textview where the current time will be shown

    Date time; // The time since this clock was started
    SimpleDateFormat timeFormat;// The format that will be applied when
     ...
     * @param format
     *            The format to print the current time in. Supports the same
     *            formats that SimpleDateFormat supports.
     */
    public TimeTick(TextView clockFace, String format) {
	this.clockFace = clockFace;
	time = new Date();
	timeFormat = new SimpleDateFormat(format);
	TimeZone tz = new SimpleTimeZone(0, ""); // Make sure the time is
						 // centered around 0
    ...
    /**
     * Get the time this clock is counting up from relative to the Unix Epoch
     * Time.
     */
    public long getBase() {
	return epochTime;
    ...
    /**
     * Get the amount of time (in milliseconds) that has passed since this clock
     * was started
     */
    public long getTime() {
